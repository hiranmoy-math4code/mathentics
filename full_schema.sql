...
-- FILE 1: FULL SCHEMA (Tables, Constraints, Indexes, RLS Policies)
-- ============================================================================
-- This file contains all table definitions, foreign keys, indexes, and
-- Row Level Security policies needed to restore the database functionality.
...
CREATE TABLE IF NOT EXISTS public.exams (
    ...
    CONSTRAINT exams_admin_id_fkey FOREIGN KEY (admin_id) REFERENCES public.profiles(id) ON DELETE CASCADE,
    CONSTRAINT exams_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id) ON DELETE SET NULL
);

-- Add foreign key from lessons to exams
ALTER TABLE public.lessons ADD CONSTRAINT lessons_exam_id_fkey FOREIGN KEY (exam_id) REFERENCES public.exams(id) ON DELETE SET NULL;
...
-- Text Search Index (for course titles)
